...
export function getReminder(dispatch: Dispatch) {
    ...
    reminderService.getReminder().then(({ data }) => {
        dispatch({
            type: `${REMINDER_ACTIONS.GET_REMINDER}_SUCCESS`,
            payload: data
        })
    }).catch((err:Error) => {
        dispatch({
            type: `${REMINDER_ACTIONS.GET_REMINDER}_ERROR`,
            error: err,
        })
    })
}
...
export function addReminder(dispatch: Dispatch) {
    ...
    return function (data: any) {
        reminderService.addReminder(data)
            .then(resp => {
                dispatch({
...
export function updateReminder(dispatch: Dispatch) {
    return function (data: any) {
        reminderService.updateReminder(data)
            .then(resp => {
                dispatch({
...
export function removeReminder(dispatch: Dispatch) {
    return function (data: any) {
        reminderService.deleteReminder(data)
            .then(resp => {
                dispatch({
